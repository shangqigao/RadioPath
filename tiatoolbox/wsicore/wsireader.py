...
def is_isyntax(path: pathlib.Path) -> bool:
    """Check if the input is a Isyntax file.

    Args:
        path (pathlib.Path): Path to the file to check.

    Returns:
        bool: True if the file is a isyntax file.

    """
    path = pathlib.Path(path)
    is_isyntax = path.suffix.lower() == ".isyntax"
    is_isyntax_dir = path.is_dir() and any(
        p.suffix.lower() == ".isynatx" for p in path.iterdir()
    )
    return is_isyntax or is_isyntax_dir
...
class WSIReader:
    ...
    def open(  # noqa: A003
        ...
        **kwargs,
        ...
        if is_isyntax(input_path):
            return IsyntaxWSIReader(input_path, mpp=mpp, power=power)
    ...
    def verify_supported_wsi(input_path: pathlib.Path) -> None:
        ...
        """
        if is_ngff(input_path) or is_dicom(input_path) or is_isyntax(input_path):
            return
...
class IsyntaxWSIReader(WSIReader):
    """Reader for isyntax whole-slide images.


    Attributes:
        isyntax_wsi (:obj:`IsyntaxReader`)
    ...
    def read_rect(
        ...
        **kwargs,
        ...
        wsi = self.isyntax_wsi

        # Read at optimal level and corrected read size
        im_region = wsi._read_region(location, read_level, level_size)
        im_region = np.array(im_region)
    ...
    def read_bounds(
        ...
        **kwargs,
        ...
        location_at_baseline = bounds_at_baseline[:2]
        _, size_at_read_level = utils.transforms.bounds2locsize(bounds_at_read_level)
        im_region = wsi._read_region(
            location_at_baseline, read_level, size_at_read_level
        )
        im_region = np.array(im_region)
    ...
    def _info(self):
        ...
        """
        objective_power = None
        slide_dimensions = self.isyntax_wsi.level_dimensions[0]
        level_count = self.isyntax_wsi.level_count
        level_dimensions = self.isyntax_wsi.level_dimensions
